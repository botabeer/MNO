...
def handle_message(event):
    ...
    try:
        ...
        # الأوامر النصية
            ...
            if QUESTIONS:
                line_bot_api.reply_message(event.reply_token,
                    TextSendMessage(text=f"❓ {random.choice(QUESTIONS)}", 
                        quick_reply=get_quick_reply()))
            else:
                line_bot_api.reply_message(event.reply_token,
            ...
            if CHALLENGES:
                line_bot_api.reply_message(event.reply_token,
                    TextSendMessage(text=f"🏆 {random.choice(CHALLENGES)}", 
                        quick_reply=get_quick_reply()))
            else:
                line_bot_api.reply_message(event.reply_token,
            ...
            if CONFESSIONS:
                line_bot_api.reply_message(event.reply_token,
                    TextSendMessage(text=f"💬 {random.choice(CONFESSIONS)}", 
                        quick_reply=get_quick_reply()))
            else:
                line_bot_api.reply_message(event.reply_token,
            ...
            if MORE_QUESTIONS:
                line_bot_api.reply_message(event.reply_token,
                    TextSendMessage(text=f"➕ {random.choice(MORE_QUESTIONS)}", 
                        quick_reply=get_quick_reply()))
            else:
                line_bot_api.reply_message(event.reply_token,
        ...
        elif text in ['نصيحة', 'tip']:
            if TIPS:
                categories = list(TIPS.keys())
                category = random.choice(categories)
                tip = random.choice(TIPS[category])
                line_bot_api.reply_message(event.reply_token,
                    TextSendMessage(text=f"💡 {tip}", quick_reply=get_quick_reply()))
            else:
                line_bot_api.reply_message(event.reply_token,
                    TextSendMessage(text="❌ ملف النصائح غير متوفر", quick_reply=get_quick_reply()))
            return
