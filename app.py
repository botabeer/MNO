...
def init_line_api():
    ...
    try:
        ...
        logger.info("LINE API initialized successfully")
...
def push_message(to, messages):
    ...
    try:
        ...
        logger.exception(f"Failed to push message to {to}")
    ...
    try:
        ...
        if user_id in waiting_for_name_change:
            ...
            try:
                ...
                logger.exception(f"Failed to update user name {user_id}")
        ...
        if not is_bot_command(text):
            ...
            if game:
                ...
                display_name = stats.get('display_name', 'مستخدم') if stats else "مستخدم"
                ...
                if result:
                    ...
                    if result.get('correct') and result.get('points', 0) > 0 and game_type not in ['mafia', 'compatibility']:
                        try:
                            ...
                            logger.exception(f"Failed to update points for {user_id}")
        ...
        # Update last activity for registered users
        try:
        ...
        if normalized_text.lower() in {"بدايه", "start", "ابدا", "بداية"}:
            flex = FlexMessage(
                ...
                contents=FlexContainer.from_dict(
                    UIBuilder.welcome_card(display_name, Database.is_user_registered(user_id))
                )
            )
            reply_message(event.reply_token, flex)
            return
        ...
        if normalized_text in {"ألعاب", "العاب"}:
            flex = FlexMessage(
                ...
                contents=FlexContainer.from_dict(
                    UIBuilder.games_menu_card(Database.is_user_registered(user_id))
                )
            )
            reply_message(event.reply_token, flex)
            return
            ...
            try:
                if Database.is_user_registered(user_id):
                    flex = FlexMessage(
                        ...
                        contents=FlexContainer.from_dict(
                            UIBuilder.already_registered_card(display_name)
                        )
                    )
                    reply_message(event.reply_token, flex)
                else:
                    ...
                    if existing_name:
                        ...
                        flex = FlexMessage(
                            ...
                            contents=FlexContainer.from_dict(
                                UIBuilder.welcome_back_card(existing_name)
                            )
                        )
                        reply_message(event.reply_token, flex)
                    else:
            ...
            try:
                if not Database.is_user_registered(user_id):
                    ...
                    flex = FlexMessage(
                        ...
                        contents=FlexContainer.from_dict(
                            UIBuilder.change_name_card(display_name)
                        )
                    )
                    reply_message(event.reply_token, flex)
            except Exception:
        ...
        if normalized_text == "انسحب":
            try:
                if not Database.is_user_registered(user_id):
                    ...
                    reply_message(
                        event.reply_token, 
                        TextMessage(text="تم الانسحاب. لن يتم احتساب اجاباتك بعد الآن")
                    )
            except Exception:
        ...
        if normalized_text in {"نقاطي", "احصائياتي"}:
            try:
                ...
                flex = FlexMessage(
                    alt_text="احصائياتك",
                    contents=FlexContainer.from_dict(
                        UIBuilder.stats_card(display_name, stats)
                    )
                )
                reply_message(event.reply_token, flex)
            except Exception:
        ...
        # Fun commands (no registration required)
        ...
        # Compatibility game (no registration required)
        ...
        # Game commands (require registration)
        game_commands = {
            "اغنيه": "song",
            "لعبه": "human_animal",
            "سلسله": "chain",
        ...
        if normalized_text in game_commands:
            # Check registration except for special games
            if normalized_text not in {"مافيا", "توافق"}:
                try:
                    if not Database.is_user_registered(user_id):
                        reply_message(
                            event.reply_token, 
                            TextMessage(text="يجب التسجيل اولا لبدء الالعاب")
                        )
                        return
                except Exception:
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting locally on port {port}")
    app.run(
        host='0.0.0.0',
        port=port,
