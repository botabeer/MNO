...
class CategoryLetterGame:
    ...
    def start_game(self):
        self.questions = random.sample(
            self.challenges,
            min(self.total_questions, len(self.challenges))
        )
        self.current_question = 0
        self.player_scores = {}
        self.answered_users = set()
    ...
    def check_answer(self, text, user_id, display_name):
        ...
        if normalized in valid_answers:
            ...
            if self.current_question + 1 < self.total_questions:
                return {
                    'response': TextMessage(
                        text=f"اجابة صحيحة {display_name}\n+1 نقطة"
                    ),
                    'points': 1,
                    'correct': True,
                    'next_question': True
    ...
    def _end_game(self):
        ...
        return {
            'response': FlexMessage(
                ...
                contents=FlexContainer.from_dict(
                    create_winner_card(winner, sorted_players, "فئة")
                )
            ),
            'points': winner['score'],
            'correct': True,
