        ...
        if len(lines) >= 4:
            ...
            if len(words) >= 4:
                ...
                if valid_count >= 1:
                    ...
                    if self.current_question + 1 < self.total_questions:
                        return {'response': TextMessage(text=f"صحيح {display_name} الكلمات الصحيحة: {valid_count}/4 +{points}"), 'points': points, 'correct': True, 'won': valid_count == 4, 'next_question': True}
                    return self._end_game()
        return None
